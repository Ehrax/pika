# Billbi Scaffold Architecture Design

## Summary

Create the project foundation for Billbi without implementing the product UI or behavior yet.
...
- Remove or isolate the starter template app shape so future work starts from Billbi-specific foundations.
...
```text
billbi/App/BillbiApp.swift
billbi/App/AppDependencyGraph.swift
```
...
```text
billbi/DesignSystem/BillbiColor.swift
billbi/DesignSystem/BillbiTypography.swift
billbi/DesignSystem/BillbiSpacing.swift
billbi/DesignSystem/BillbiRadius.swift
billbi/DesignSystem/BillbiStatusTone.swift
```
...
```text
billbi/Navigation/AppRoute.swift
billbi/Navigation/SheetDestination.swift
billbi/Navigation/AppRouter.swift
```
...
```text
billbi/Models/ProjectRecord.swift
```

Responsibilities:

- Replace the starter `Item` demo model with one Billbi-named record.
...
billbi/Services/AppSettings.swift
```
...
```text
billbi/Shell/RootView.swift
```

Responsibilities:

- Present a placeholder Billbi root surface.
...
```text
billbi/Support/MoneyFormatting.swift
...
- Include only the minimal Billbi-named model.
...
- SwiftData is wired with a minimal Billbi-specific model.
...
- No real Billbi product screens or workflows are implemented.
