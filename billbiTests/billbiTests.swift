...
struct BillbiScaffoldTests {
    @Test func designTokensExposeExpectedScaffoldValues() {
        #expect(BillbiSpacing.md == 16)
        #expect(BillbiRadius.lg == 8)
    ...
    @Test func sidebarProjectRowsUseFullWidthSelectionWithChildIndentation() {
        ...
        #expect(SidebarProjectRowLayout.contentHorizontalPadding == BillbiSpacing.sm)
    ...
    @Test func appModelContainerCanPersistNormalizedRecordsInMemory() throws {
        let container = try BillbiApp.makeModelContainer(mode: .inMemory)
    ...
    @Test func macOSLaunchWindowPolicyStartsWithRoomForSidebarAndContent() {
        #expect(BillbiApp.defaultLaunchWindowSize.width == 1408)
        #expect(BillbiApp.defaultLaunchWindowSize.height == 813)
        #expect(MainWindowLayout.frameAutosaveName == "BillbiMainWindowFrame")
        #expect(MainWindowLayout.frameStorageKey == "billbi.mainWindow.frame")
    }
    ...
    @Test func macOSPrimarySidebarPolicyStartsWithBreathingRoom() {
        ...
        #expect(PrimarySidebarColumnLayout.widthStorageKey == "billbi.primarySidebar.width")
    }
    ...
    @Test func resizableDetailSplitPolicyPersistsWideSecondarySidebar() {
        ...
        #expect(ResizableDetailSplitLayout.leadingWidthStorageKey == "billbi.resizableDetailSplit.leadingWidth")
    }

    @Test func secondarySidebarHeaderAvoidsMacWindowChromeWhenItBecomesLeadingColumn() {
        #expect(BillbiSecondarySidebarLayout.headerTopPadding >= BillbiSpacing.md)
        #expect(BillbiSecondarySidebarLayout.leadingChromeClearance(forColumnMinX: 0) >= 116)
        #expect(BillbiSecondarySidebarLayout.leadingChromeClearance(forColumnMinX: 260) == 0)
    }
    ...
    @Test func appEnvironmentResolvesDevelopmentMetadata() throws {
        let environment = try AppEnvironment.resolve(
            infoDictionary: [
                ...
                AppEnvironment.cloudKitContainerIdentifierInfoDictionaryKey: "iCloud.ehrax.dev.billbi.dev",
            ]
        )
    ...
    @Test func appEnvironmentResolvesProductionMetadata() throws {
        let environment = try AppEnvironment.resolve(
            infoDictionary: [
                ...
                AppEnvironment.cloudKitContainerIdentifierInfoDictionaryKey: "iCloud.ehrax.dev.billbi",
            ]
        )
    ...
    @Test func cloudKitPrivatePersistenceUsesResolvedEnvironmentContainer() {
        ...
        let configuration = AppPersistenceMode.cloudKitPrivate.makeModelConfiguration(
            schema: schema,
            storeURL: BillbiApp.defaultStoreURL(
                mode: .cloudKitPrivate,
                appEnvironment: .development,
                bundleIdentifier: "ehrax.dev.billbi.dev"
            ),
            appEnvironment: .development
        )

        #expect(String(describing: configuration).contains("iCloud.ehrax.dev.billbi.dev"))
    }

    @Test func persistentStoreURLsAreSeparatedByEnvironmentAndBundleIdentifier() throws {
        let devStoreURL = try #require(BillbiApp.defaultStoreURL(
            mode: .cloudKitPrivate,
            appEnvironment: .development,
            bundleIdentifier: "ehrax.dev.billbi.dev"
        ))
        let prodStoreURL = try #require(BillbiApp.defaultStoreURL(
            mode: .cloudKitPrivate,
            appEnvironment: .production,
            bundleIdentifier: "ehrax.dev.billbi"
        ))

        #expect(devStoreURL != prodStoreURL)
        #expect(devStoreURL.path().contains("/Billbi/dev/ehrax.dev.billbi.dev/"))
        #expect(prodStoreURL.path().contains("/Billbi/prod/ehrax.dev.billbi/"))
    }

    @Test func inMemoryPersistenceDoesNotResolvePersistentStoreURL() {
        let storeURL = BillbiApp.defaultStoreURL(
            mode: .inMemory,
            appEnvironment: .development,
            bundleIdentifier: "ehrax.dev.billbi.dev"
        )
    ...
    @Test func workspaceBackupsDirectoryIsSeparatedByEnvironmentAndBundleIdentifier() throws {
        ...
        #expect(backupsURL.path().contains("/Billbi/dev/ehrax.dev.billbi.dev/Backups"))
    }
    ...
    @Test func appLaunchConfigurationUsesLocalModeForExplicitSeedResets() {
        let configuration = AppLaunchConfiguration(
            arguments: ["billbi", "--billbi-workspace-seed", "sample"],
            environment: [:],
            isRunningTests: false
        )
    ...
    @Test func appLaunchConfigurationUsesExplicitLocalPersistenceOverride() {
        let configuration = AppLaunchConfiguration(
            arguments: ["billbi", "--billbi-persistence", "local"],
            environment: [:],
            isRunningTests: false
        )
    ...
    @Test func appLaunchConfigurationUsesExplicitInMemoryPersistenceOverride() {
        let configuration = AppLaunchConfiguration(
            arguments: ["billbi"],
            environment: ["BILLBI_PERSISTENCE": "in-memory"],
            isRunningTests: false
        )
    ...
    @Test func buildAndRunScriptDoesNotUseLegacyWorkspaceStorePathOverrides() throws {
        ...
        #expect(script.contains("--billbi-workspace-store-path") == false)
        #expect(script.contains("BILLBI_WORKSPACE_STORE_PATH") == false)
        #expect(script.contains("--prod") == true)
        #expect(script.contains("--billbi-persistence") == true)
    }
    ...
    @Test func prodArtifactScriptBuildsFinderOnlyLocalAndCloudArtifacts() throws {
        ...
        #expect(script.contains("BillbiProdLocal") == true)
        #expect(script.contains("BillbiProdCloud") == true)
        #expect(script.contains("LSEnvironment:BILLBI_PERSISTENCE") == true)
    ...
    @Test func appLaunchConfigurationUsesInMemoryModeForExplicitUITestEnvironment() {
        let configuration = AppLaunchConfiguration(
            arguments: ["billbi"],
            environment: ["BILLBI_UI_TESTING": "1"],
            isRunningTests: false
        )
    ...
    @Test func appLaunchConfigurationParsesLegacyDemoWorkspaceSeedFlag() {
        let configuration = AppLaunchConfiguration(
            arguments: ["billbi", "--billbi-seed-workspace"],
            environment: [:]
        )
    ...
    @Test func appLaunchConfigurationParsesBikeparkWorkspaceSeed() {
        let configuration = AppLaunchConfiguration(
            arguments: ["billbi", "--billbi-workspace-seed", "bikepark-thunersee"],
            environment: [:]
        )
    ...
    @Test func appLaunchConfigurationParsesEnvironmentWorkspaceSeed() {
        let configuration = AppLaunchConfiguration(
            arguments: ["billbi"],
            environment: ["BILLBI_WORKSPACE_SEED": "sample"]
        )
    ...
    @Test func cloudKitSyncConfigurationIncludesRequiredCapabilitiesAndPersistenceTelemetryHooks() throws {
        let repositoryRoot = URL(filePath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let entitlementsURL = repositoryRoot.appendingPathComponent("billbi/billbi.entitlements")
        ...
        #expect(entitlements.contains("<string>$(BILLBI_CLOUDKIT_CONTAINER_IDENTIFIER)</string>"))
