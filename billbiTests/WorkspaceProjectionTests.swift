...
struct WorkspaceProjectionTests {
    ...
    @Test func projectBucketRowsSortByWorkflowStateWithOpenBucketsMostRecentlyEdited() throws {
        ...
        let project = WorkspaceProject(
            ...
            buckets: [
                ...
                bucket(id: openOlderID, name: "Open older", status: .open, updatedAt: Date.billbiDate(year: 2026, month: 4, day: 20)),
                bucket(id: openNewerID, name: "Open newer", status: .open, updatedAt: Date.billbiDate(year: 2026, month: 4, day: 24)),
            ],
            invoices: [
                invoice(id: 1, projectID: projectID, bucketID: finalizedID, bucketName: "Finalized", status: .sent, dueDate: Date.billbiDate(year: 2026, month: 5, day: 10)),
                invoice(id: 2, projectID: projectID, bucketID: overdueID, bucketName: "Overdue", status: .sent, dueDate: Date.billbiDate(year: 2026, month: 4, day: 1)),
                invoice(id: 3, projectID: projectID, bucketID: paidID, bucketName: "Paid", status: .paid, dueDate: Date.billbiDate(year: 2026, month: 4, day: 1)),
            ]
        )
    ...
    @Test func dashboardRevenueHistoryReflectsPaidInvoicesByIssueMonth() {
        ...
        workspace.projects[projectIndex].invoices.append(
            WorkspaceInvoice(
                ...
                issueDate: Date.billbiDate(year: 2026, month: 4, day: 26),
                dueDate: Date.billbiDate(year: 2026, month: 5, day: 12),
                status: .finalized,
    ...
    @Test func dashboardRevenueRangeSevenDaysUsesDailyBucketsEndingOnCurrentDate() {
        let points = [
            RevenuePoint(date: Date.billbiDate(year: 2026, month: 3, day: 16), label: "March", amountMinorUnits: 125_000),
            RevenuePoint(
    ...
    @Test func projectCardsSelectTheMatchingProjectShellDestination() throws {
        ...
        #expect(BillbiShellDestination.projectDestination(for: project) == .project(project.id))
    }
